...
> **원칙**: 새로운 API 엔드포인트는 기본적으로 세 가지를 동시에 만족해야 한다 — `current_user` 인증, 리소스 `user_id` 소유권 검증, RLS 적용(anon key 클라이언트). 이 세 가지 중 하나라도 생략할 경우, 명시적인 `public` 태그와 문서화가 필요하다. (사례: V-01~V-04)
