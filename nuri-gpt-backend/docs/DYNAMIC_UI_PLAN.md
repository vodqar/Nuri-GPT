# 동적 템플릿 UI 렌더링
  ...
  > 이 문서는 동적 렌더링 아이디어를 정리한 계획 문서입니다. 현재 구현의 세부 계약이나 화면 규칙이 바뀌었을 수 있으므로, 실제 작업 시에는 최신 `agents.md`, Backend/Frontend의 관련 `docs/`, 그리고 현재 코드 구조를 함께 확인합니다.

  > 구조적 제약이 뒤늦게 드러났다면 이 문서를 그대로 따르기보다, 어느 계층이 안정된 구조를 보장할지 먼저 정리한 뒤 관련 상위 문서로 승격합니다.
